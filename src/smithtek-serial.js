...
module.exports = function(RED) {
  ...
  // TODO: 'serialPool' should be encapsulated in SerialPortNode
            ...
            node.on("input",function(msg) {
                if (msg.hasOwnProperty("payload")) {
                    ...
                    if (!Buffer.isBuffer(payload)) {
                        ...
                        payload = Buffer.concat([payload,Buffer.from(node.addCh)]);
                    }
    ...
    RED.nodes.registerType("smithtek-serial-out",SerialOutNode);
    ...
    function SmithtekSerialInNode(n) {
        ...
        if (this.serialConfig) {
            ...
            node.tout = null;
            var buf;
            if (node.serialConfig.out != "count") { buf = Buffer.alloc(bufMaxSize); }
            else { buf = Buffer.from(Number(node.serialConfig.newline)); }
            var i = 0;
            ...
            if (node.serialConfig.newline.substr(0,2) == "0x") {
                splitc = Buffer.from([parseInt(node.serialConfig.newline)]);
            } else {
                splitc = Buffer.from(node.serialConfig.newline.replace("\\n","\n").replace("\\r","\r").replace("\\t","\t").replace("\\e","\e").replace("\\f","\f").replace("\\0","\0")); // jshint ignore:line
            }
            ...
            this.port.on('data', function(msg) {
                ...
                if ((node.serialConfig.newline === 0)||(node.serialConfig.newline === "")) {
                    ...
                    else { node.send({"payload": Buffer.from([msg])}); }
                }
                else {
                    // do the timer thing
